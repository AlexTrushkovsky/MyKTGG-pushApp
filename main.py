...
# Form implementation generated from reading ui file 'myktggpusher.ui'
#
...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "MyKTGG Pusher"))
    ...
    def getTopics(self):
        ...
        arr.append("test")
        arr.append("news")
        arr += self.getListOfUsers(True)
        arr += self.getListOfUsers(False)
    ...
    def getListOfUsers(self, isStudent: bool):
        newArr = []
        if isStudent:
            URL = "http://app.ktgg.kiev.ua/cgi-bin/timetable_export.cgi?req_type=obj_list&req_mode=group&req_format=json&coding_mode=WINDOWS-1251&bs=ok"
        else:
            URL = "http://app.ktgg.kiev.ua/cgi-bin/timetable_export.cgi?req_type=obj_list&req_mode=teacher&req_format=json&coding_mode=WINDOWS-1251&bs=ok"
        try:
            r = requests.get(URL)
            if r.status_code == requests.codes.ok:
                ...
                if isStudent:
                    for item in array:
                        if len(item) <= 10:
                            newArr.append("".join(item.rstrip().lstrip()))
                else:
                    for item in array:
                        if item.count(".") >= 2:
                            newArr.append("".join(item.rstrip().lstrip()))
        except:
            self.showMessage(QMessageBox.Warning, "Не вдається під'єднатися до серверу, перевірте з'єднання з мережею.","Помилка")
        return newArr
